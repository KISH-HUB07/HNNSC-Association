...
const AdminDashboard = () => {
    ...
    const fetchAuctionItems = async (year) => {
        try {
            ...
            const q = query(auctionRef, where('year', '==', year));
            const snapshot = await getDocs(q);
            setAuctionItems(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        } catch (error) { console.error(error); showToast('Failed to fetch auction items', 'error'); }
    };

    // ... (Continue with your existing handler functions below this line)

    const handleDeleteEvent = async (eventId) => {
        if (!window.confirm('Are you sure you want to delete this event? All registrations for this event will also be deleted.')) {
            return;
        }
        try {
            ...
            showToast('Event and its registrations deleted successfully!', 'success');
    ...
    const handleAddAuctionItem = async (e) => {
        ...
        try {
            const newItem = {
                ...
                buyerName: auctionForm.buyerName,
            ...
            await addDoc(collection(db, 'auction'), newItem);
    ...
    const exportMembersToExcel = () => {
        ...
        XLSX.writeFile(workbook, `HNNSC_Members_${new Date().toISOString().split('T')[0]}.xlsx`);
                    ...
                    <div className="members-section">
                        ...
                        {members.length > 0 && (
                            <div className="search-bar-container">
                                <input
                                    type="text"
                                    placeholder="🔍 Search by name, email, phone, kovil, pirivu, native place, patta per..."
